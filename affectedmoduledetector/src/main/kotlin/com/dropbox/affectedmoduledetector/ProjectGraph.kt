...
internal class ProjectGraph(project: Project, val gitRoot: File, val logger: Logger? = null) {
    ...
    init {
        ...
        project.subprojects.forEach {
            ...
            // If the subproject is not a child of the root project (in the File directory sense) then we are in some
            // weird non standard quixotic project and our sections are going to have ".." characters indicating that
            // we need to traverse up one level. However we need to filter these out because this will not match the
            // parent child dependency relationship that Gradle will produce.
